...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // What if there will be access token & refresh, backend will throw unauthorzied even if u pass by cookies refresh?
    // Frontend should send only refresh (in cookie) for routes when we want to verify refresh token to avoid this problem
